...
namespace SDKTemplate
{
    public sealed partial class MyUserControl1 : Page
    {
        ...
        //public ScrollViewer GetScrollViewer { get => this.myViewer; }
        public ScrollViewer scrollViewer { get => this.myViewer; set => myViewer = value; }
        //public CaptureElement CaptureElement { get => this.captureElement; }
        ...
        private async Task UpdateButtonStateAsync()
        {
            await Dispatcher.RunAsync(Windows.UI.Core.CoreDispatcherPriority.Normal, () =>
            {
                ...
                catch (Exception e) { var temp = e.Message; }
            });
        }
                        ...
                        if (QueryPerformanceCounter(out currtime))
                        {
                            if (QueryPerformanceFrequency(out freq))
                            {
                                ...
                                if (++s_cnt == 30)
                                {
                                    ...
                                    delta = s_ullDeltaSum / s_cnt;
                                    _logger.Log("delta = " + delta + ", fps = " + fps + ", deltaInternal = " + s_ullDeltaSumInternal / s_cnt + "\n");
                                    string[] data = { fps.ToString(), delta.ToString() };

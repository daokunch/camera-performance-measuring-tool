...
const __int64 nano100SecInMSec = (__int64)10000;
...
/* return count of period between pst1 and pst2*/
...
void Scenario2_FindAvailableSourceGroups::Reader_FrameArrived(MediaFrameReader^ reader, MediaFrameArrivedEventArgs^ args)
{
    ...
    if (MediaFrameReference^ frame = reader->TryAcquireLatestFrame())
    {
        ...
        double interval = DT_PeriodsBetween(nano100SecInMSec, &prev_st, &st);

        double fps = 1000 / interval;


        //m_logger->Log("The time diff is : " + interval + " ms\n");

        //m_logger->Log("The fps is : " + fps + "\n");

        mean_fps = (mean_fps * frames + fps) / (frames + 1);
        ...
        if(!(frames % 30))
        {
            ...
            m_logger->Log("The mean fps is : " + mean_fps + "\n");
